...
The system SHALL allow a client to add a `Book` to the catalog via `POST /books` with a
...
- **THEN** the system returns `201 Created`, a `Location` header of `/books/{id}`, and a
...
The system SHALL return a single book via `GET /books/{id}`.

#### Scenario: Book exists

- **WHEN** a client GETs `/books/{id}` for an existing id
- **THEN** the system returns `200 OK` with the book's full representation

#### Scenario: Book does not exist

- **WHEN** a client GETs `/books/{id}` for an id that is not in the catalog
- **THEN** the system returns `404 Not Found`

### Requirement: List and filter books

The system SHALL return a paginated list of books via `GET /books`, accepting `page` and
`size` parameters. It SHALL support optional filtering by `title` (case-insensitive
...
The system SHALL delete a book via `DELETE /books/{id}`.

#### Scenario: Successful deletion

- **WHEN** a client DELETEs `/books/{id}` for an existing id
- **THEN** the system returns `204 No Content` and the book is no longer retrievable

#### Scenario: Delete a non-existent book

- **WHEN** a client DELETEs `/books/{id}` for an id that is not in the catalog
- **THEN** the system returns `404 Not Found`
